...
from utils.colmap_loader import read_extrinsics_text, read_intrinsics_text, qvec2rotmat, \
...
# For blender data
def readNerfSyntheticInfo(path, white_background = True, eval = False, extension=".png"):
...
def readColmapSceneInfo(path, eval = False, llffhold=8):
    ...
    # reconstructed 3D points, use (x,y,z) and (R,G,B) in initial Gaussian
    bin_path = os.path.join(path, "sparse/0/points3D.bin")
    positions, colors, _ = read_points3D_binary(bin_path)
    normals = np.zeros_like(positions)

    pcd = BasicPointCloud(points=positions, colors=colors, normals=normals)

    scene_info = SceneInfo(point_cloud=pcd,
                           train_cameras=train_cam_infos, # uid,R, T, FovY, FovX, image, image_path,...,width, height
                           test_cameras=test_cam_infos,
                           nerf_normalization=nerf_normalization)
    return scene_info
...
#
# # train_cam_infos = scene_info.train_cameras
# # train_cam_infos[0].image_path
#
# from utils.gaussian_model import GaussianModel
# import torch
# gaussian = GaussianModel(3)
# device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
# gaussian.create_from_pcd(scene_info.point_cloud,device)
# print("_xyz.shape: {}\n _f_dc.shape: {}\n _f_rest.shape: {}\n scaling.shape: {}\n rotation.shape: {} \
#       \n opacity.shape: {}\n" \
#       .format(gaussian._xyz.shape,gaussian._features_dc.shape,gaussian._features_rest.shape,
#               gaussian._scaling.shape,gaussian._rotation.shape,gaussian._opacity.shape))
