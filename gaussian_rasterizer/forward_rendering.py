...
class GaussRender:
    ...
    Render the scene.
    """
    def __init__(self, active_sh_degree=3, white_bkgd=True, device = "cpu"):
    ...
    def rasterizer(self, camera, means2D, cov2d, color, opacity, depths, TILE_SIZE = 40,device = "cpu"):
        radii = get_radius(cov2d)
        rect = get_rect(means2D, radii, width=camera.image_width, height=camera.image_height)

        self.pix_coord = torch.stack(torch.meshgrid(torch.arange(camera.image_height), torch.arange(camera.image_width), indexing='xy'), dim=-1).to(
            self.device)
    ...
    def render(self,viewpoint_camera, pc : GaussianModel):
        ...
        # start Render
        # radii = get_radius(cov2d)
        # print(means2D.shape, cov2d.shape)
