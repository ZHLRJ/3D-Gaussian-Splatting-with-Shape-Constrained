...
class Viewer(object):
    ...
    def _define_guis(self):
        ...
        with self.server.gui.add_folder("Control"):
            self.gui_w = self.server.gui.add_number(
                "w",
                initial_value=(0.0),
                step=0.01,
            )

            self.gui_xyz = self.server.gui.add_vector3(
                "xyz",
                initial_value=(1.0, 1.0, 1.0),
                step=0.01,
            )

            self.gui_pos = self.server.gui.add_vector3(
                "pos",
                initial_value=(1.0, 1.0, 1.0),
                step=0.01,
            )

            @self.gui_w.on_update
            def _(_) -> None:
                clients = self.server.get_clients()
                for id, client in clients.items():
                    client.camera.wxyz = np.array([self.gui_w.value, self.gui_xyz.value[0], self.gui_xyz.value[1], self.gui_xyz.value[
                        2]])

            @self.gui_xyz.on_update
            def _(_) -> None:
                clients = self.server.get_clients()
                for id, client in clients.items():
                    client.camera.wxyz = np.array([self.gui_w.value, self.gui_xyz.value[0], self.gui_xyz.value[1], self.gui_xyz.value[
            ...
            @self.gui_pos.on_update
            def _(_) -> None:
                clients = self.server.get_clients()
                for id, client in clients.items():
                    client.camera.position = self.gui_pos.value
